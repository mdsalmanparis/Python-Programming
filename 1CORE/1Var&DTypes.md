...
Break these and you get a `SyntaxError` or `NameError`:
...
**snake_case** is Python's standard style (PEP 8):
...
Unlike C or Java where `int` overflows, Python integers grow as large as memory allows:
...
Single and double quotes are identical in Python. Choose one and be consistent:
...
This is not obvious but important:
...
**Uninitialized variables — placeholder pattern:**
```python
user = None
# ... later in code ...
...
### Critical Point: int() Truncates, Not Rounds
...
User input always comes as string, output always needs string:
...
These are ALL False — memorise this list:
...
Use parentheses whenever there's any doubt. Explicit is better than relying on someone knowing precedence rules.
...
x /= 4     # same as x = x / 4  →  x is now 5.0
x //= 2    # same as x = x // 2 →  x is now 2.0
...
Important: Python is lazy. It stops evaluating as soon as the result is determined.
...
## and and or Return the Actual Value

Not just True/False — this surprises most people. Python doesn't return `True`/`False` — it returns the value that determined the outcome.
